...
``get_instance_hash()`` hashes the dict returned by
``get_instance_state()``, which serialises the ``params`` dict:
...
``Artifact`` overrides ``get_instance_state()`` to add a
``processor_hash`` entry when a processor is set.
...
parameter is itself an ``Artifact``.
