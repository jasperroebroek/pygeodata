...
from pygeodata.registry_browser.models import ClassInfo

logger = logging.getLogger(__name__)


def source_info_from_disk(class_name: str) -> tuple[str, list[str], str | None, str | None, str | None, str | None]:
    """Read object_type, dependency_names, source_path, graph_path, stored_source_hash, registry_path from source.json.

    Used for classes that are known from cache but not loaded into the Python registry.
    Returns ('unknown', [], None, None, None, None) if nothing is found.
    """
...
def merge_unloaded_classes(
    ...
    for class_name, group in groups.items():
        ...
        object_type, dep_names, source_path, graph_path, _, registry_path = source_info_from_disk(class_name)
