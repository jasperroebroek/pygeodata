...
from .models import LinkedEntry, ParamRow

HIDDEN_KEYS = frozenset(value for key, value in vars(JSONKeys).items() if not key.startswith('__'))
PARAMS_KEY = 'params'
...
def _is_data_ref(value: Any) -> bool:
    return (
        ...
        and JSONKeys.CLASS_NAME in value
        and PARAMS_KEY in value
    )
...
def _flatten(
    ...
    if _is_data_ref(value):
        ...
        if linked_entries is not None:
            linked_entries.append(LinkedEntry(
                param_name=key,
