...
from .path_actions import open_path, reveal_path
from .payloads import build_dashboard_payload
from .popups import build_graph_popup, build_json_popup, build_source_popup
...
def _assert_allowed_path(path_str: str) -> Path:
...
def api_popup_source():
    return jsonify(build_source_popup(
...
def api_popup_graph():
    return jsonify(build_graph_popup(
        request.args['class_name'],
...
def api_bounds_map():
    ...
    bounds = _json.loads(request.args['bounds'])   # [lat_min, lon_min, lat_max, lon_max]
    ...
    label = f"{sw} → {ne}"
    return render_template('bounds_map.html',
