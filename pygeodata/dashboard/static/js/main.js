...
$("#modal-body").addEventListener("click", (e) => {
  ...
  if (link) {
    ...
    closeModal();
    pushHistory(_viewMode);
    if (!state.selected_classes.includes(cn)) state.selected_classes.push(cn);
    state.selected_entry = null;
    const targetCard = (lastDashboard?.class_cards ?? []).find((c) => c.class_name === cn);
    if (targetCard) {
      const kind = targetCard.object_type.toLowerCase();
...
function renderDetail(detail) {
  ...
  el.innerHTML = [figureCard, classCard, entryCard, coOutputsCard, linkedCard, paramsCard].join("");
...
function buildCoOutputsCard(entry) {
    ...
    return `
      <div class="link-row">
        <div class="link-row-top">
          ...
          ${badge(e.object_type, "badge-neutral")}
          ${specParts}
  ...
  return `
    <div class="dcard">
      <div class="dcard-hd"><span class="dcard-hd-label">Co-outputs</span></div>
      <div class="dcard-body dcard-body--links">${rows}</div>
    </div>`;
}
