...
from .filters import Filter, entry_matches_filters, matching_rows, parse_filters
from .models import ClassInfo, EntryInfo, FileRef, LinkedEntry
...
def _entry_matches_spec_filters(entry: EntryInfo, spec_filters: dict) -> bool:
    for dim in ('crs', 'resolution', 'shape'):
    ...
    # Bounds filter uses bounds_latlon formatted as a list string
    selected_bounds = spec_filters.get('bounds')
    if selected_bounds:
...
def _co_output_payload(entry: EntryInfo) -> dict:
    return {
        'record_id':   entry.record_id,
...
def _entry_detail_payload(entry: EntryInfo) -> dict:
    return {
        'record_id':            entry.record_id,
        'state_hash':           entry.state_hash,
        ...
        'spec':                 _spec_payload(entry.spec),
        'primary_file':         _file_payload(entry.primary_file) if entry.primary_file else None,
        'figure_previews':      [entry.primary_file.path] if entry.primary_file and entry.primary_file.kind == 'image' else [],
        'co_outputs':           [_co_output_payload(e) for e in entry.co_outputs],
    ...
    if selected_entry_info is not None:
        ...
        return {
            **_class_detail_payload(class_info),
            'selected_entry': _entry_detail_payload(selected_entry_info),
        }
