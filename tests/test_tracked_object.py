...
def test_initialize_registry_creates_files() -> None:
    ...
    code_resolver = CodeRegistryResolver.from_source_hash(calculate_cls_source_hash(SimpleTrackedObject))
    tree_resolver = TreeRegistryResolver.from_dep_tree_hash(SimpleTrackedObject.get_dependency_tree_hash())
...
def test_initialize_registry_handles_real_call_cycle() -> None:
    ...
    c_tree = TreeRegistryResolver.from_dep_tree_hash(C.get_dependency_tree_hash())
    d_tree = TreeRegistryResolver.from_dep_tree_hash(D.get_dependency_tree_hash())
